...
export const progress = (isAttached, attachHead, attachTail, targetTime, scaledTime, group) => {
    if (isAttached) {
        ...
        const attachHeadImport = archetypes.NormalHeadTapNote.import.get(attachHead)
        const attachTailImport = archetypes.NormalHeadTapNote.import.get(attachTail)
        const headProgress =
            time.now < attachHeadMemory.targetTime
                ? progressTo(
                      attachHeadMemory.targetScaledTime,
                      archetypes['#TIMESCALE_GROUP'].timeToScale.get(
                          attachHeadImport.timeScaleGroup,
                      ).currentScaledTime,
                      options.noteSpeed,
                  )
                : 1
        const tailProgress = progressTo(
            ...
            archetypes['#TIMESCALE_GROUP'].timeToScale.get(attachTailImport.timeScaleGroup)
                .currentScaledTime,
            options.noteSpeed,
        )
        const headFrac =
